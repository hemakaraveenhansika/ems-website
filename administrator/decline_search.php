...
$sql = "SELECT * FROM leave_table WHERE status=4 AND CONCAT(employee_ID,employee_name) LIKE '%".$_POST["search"]."%' ORDER BY date_of_request DESC";
}else{
$sql = "SELECT * FROM leave_table WHERE status=4 AND assesd_by=$adminID AND CONCAT(employee_ID,employee_name) LIKE '%".$_POST["search"]."%' ORDER BY date_of_request DESC";
}
...
if(mysqli_num_rows($result) > 0){
    $output .= '<h4 align ="center" style="font-size:32px; font-weight: bold;">Search Result For Decline Leave</h4>';
    $output .= '<br><br>';
