...
if ($admin_row["role"]=="full_access"){
   ...
   $sql = "SELECT * FROM leave_table WHERE status=1 AND assesd_by=$adminID AND CONCAT(employee_ID,employee_name) LIKE '%".$_POST["search"]."%' ORDER BY date_of_request DESC";
    ...
    while($row =mysqli_fetch_array($result))
    {
        $output .= '
            ...
            <td>'.$row['employee_name'].'</td><td>'.$row['leave_type'].'</td><td>'.$row['leave_date_from'].'</td><td>'.$row['leave_date_to'].'</td><td>'.$row['no_of_days'].'</td><td>'.$statusList[$row['status']].'</td>
                ...
                &nbsp;
                <a href="leave_manage.php?no='.$row['leave_num'].'=3=1" onclick="return confirm(/       Are You Sure You Want To Approve ?       /)"><span class="label label-info">Approve</span></a>
                &nbsp;
                <a href="leave_manage.php?no='.$row['leave_num'].'=4=1" onclick="return confirm(/       Are You Sure You Want To decline ?       /)"><span class="label label-danger">Decline</span></a></td>
            </tr>
        ';
    }
    echo $output;
